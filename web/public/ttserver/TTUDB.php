...
class TTUDB{
	...
	function getoid($fixed=false,$item_group=''){
		return $this->getdid($fixed,$item_group);
	}
	 ...
	 * @return unknown_type
	 */
	public function  puto(&$data,$keep=true,$item_group='')
	{
		$id = &$data['id'];
		if(!$id){
			$id = $this->getdid(null,$item_group);
			$this->_t->puto($data);
			return $id;
		}
		return $this->_t->puto($data,$keep);
	...
	{	
		$max=100000;
		$keyp = $this->_u.':';
		$ret =  $this->_t->getbypre($keyp,false,$max);
		if(!$seri)
			return $ret;
		//print_r($ret);
		return $this->_t->processmap($ret);
	}
	 ...
	 */
	public function  numch($field,$num=1,$subg='')
	{
		$id = $this->getdid($field,$subg);
		return $this->_t->numch($id,(int)$num);
	}
	 ...
	 *return 改变后的值 
	 */
	public function change($f,$num,$max=0,$min=0,$subg='')
	{
		$id = $this->getdid($f,$subg);
		return $this->_t->change($id,$num,$max,$min);
	}
