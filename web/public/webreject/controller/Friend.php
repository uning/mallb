		...
		foreach( $fl as $pid ){
			...
			if($id){
				...
				if(1|| !$fdata ||  $fdata['ut']<$now - 3600){
					...
					$acc = $ftu->getdata(array('money','exp','gem','looks'));
					if(!$acc['exp'])
						continue ;
					$fdata['name'] = $finfos['name'];
					$fdata['icon'] = $finfos['icon'];
					$fdata['pid'] = $pid;
					$fdata['ut']=$now;
					$fdata['id']=$fdid;
					$fdata['dbid']=$id;
					$fdata['exp']=$acc['exp'];
					$fdata['money']=$acc['money'];
					$fdata['gem']=$acc['gem'];
					$fdata['looks']=$acc['looks'];
					$tu->puto($fdata);
				}
				$rinfos[]=$fdata;
				unset($infos[$fdid]);
...
//兼容在线的旧flash版本				
				...
				if( $type == 2 ){  //保鲜		    
					...
					if( $now - $car['t'] <= 3*$gconfig['buytime'] ){//货物未过期
		                $ret['s'] = 'notexpiration';
		                return $ret;
		            }
					$car['fresh'] = $nid;
