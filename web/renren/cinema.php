        ...
        <div class='canvas'>
		    ...
		    <div id='cinema-main'>
				...
				<div class='cinema-body'> 
					...
					<?php 
					if($mypid==$pid){ $st =  '董事长视察电影院施工情况来啦,多发帮助邀请会加快开启速度，这是肯定的';
								if($linkid){
									$st.= '<br/>链接:'.RenrenConfig::$canvas_url.'cinema.php?fid='.$linkid;
								}
								echo $st;
					}
					else if($obj['help'][$mypid])
							echo '已经获得了你的帮助';
						else if($count<$help[$oid]['need_num']){ 
								echo '需要你的帮助才能开启'.$help[$oid]['name'];
							}
						
						if(!$obj['help'][$mypid]&&$count<$help[$oid]['need_num']&&$mypid!=$pid){?>
					...
					<h2>已经有<?php echo $count;?>位董事长帮助过<xn:name uid="<?php echo $pid;?>" linked="false" shownetwork="false" />了
					<?php 
					if($count<5)
					echo "(还需要". (5-$count)."次帮助)";
					else{
						echo ',人数已够';
						if($mypid==$pid)
						?>
						<a href="<?php echo RenrenConfig::$canvas_url;?>">返回游戏开启</a>
					<?php }
					?>
					...
					echo '</tr><tr>';
					foreach ($obj['help'] as $k=>$v){?>
					<td align="center" valign="top" style="font-weight: lighter;" bordercolor="ffffff">
